...
public class EnemeManager : MonoBehaviour
{
    ...
    public List<GameObject> enemyObjects; // Fire 오브젝트 리스트
    public float activeRange = 10f; // 활성화 거리
   
    // Update is called once per frame
    void Update()
    {
        foreach (GameObject fireObject in enemyObjects) // 모든 Fire 오브젝트에 대해
        {
            if (fireObject == null) // 오브젝트가 파괴되었다면
            {
                enemyObjects.Remove(fireObject); // 리스트에서 제거
